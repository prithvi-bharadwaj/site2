...
export function LinkList({ label, items, columns = 1, variant = "compact" }: LinkListProps) {
    ...
    >
      ...
      >
        {items.map((item, i) => {
          ...
          const titleNode = (
            <span
              className={`hover-underline text-[#F4F5F8]/60 group-hover:text-[#F4F5F8] transition-colors duration-200 ${
                variant === "prose"
              ...
              {item.favicon && (
                <img
                  ...
                  className={`h-3.5 w-3.5 rounded-sm opacity-70 group-hover:opacity-100 transition-opacity ${
                    variant === "prose" ? "inline-block align-text-bottom mr-1" : ""
                  }`}
                  width={14}
                  height={14}
                />
          ...
          const wrapperClass =
            variant === "prose"
              ? "group flex flex-wrap items-baseline gap-x-2 leading-relaxed"
              : "project-item group";
                  ...
                  <div className="pt-1 pb-1 leading-relaxed">
                    ...
                    {item.href && !item.expand && (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="text-[11px] text-[#F4F5F8]/45 hover:text-[#F4F5F8]/80 underline underline-offset-2"
                      >
                        open ↗
                      </a>
                    )}
